...
SEXP DeconvolveGH_CALL(SEXP SEXP_eta, 
    ...
    for(t=0; t<q_points; t++) *(p_z_lag+t) = 0.0; 
...
void dDeltadTheta( double *S_MuiPC_lag, 
    ...
    double ddtheta_MuiPC_lag, MuiPC_lag, z, w;
    int s;
...
void LogLScoreTheta( double *S_Delta,
    ...
    int s, j;
    double Li, Li_temp, lp_c, MuC, dLi_dtheta, Y_MuC_dALL;
...
SEXP LogLScoreCalc_CALL( SEXP SEXP_betaM,
    ...
    double *Xi, *Xi_gam, *Xi_sig, *etai, *Yi, *Yilag,  *GAM,  *SIG, *Offset, *countYs;

    // SEXP variables needed
    SEXP SEXP_subject, SEXP_etai, SEXP_Deltai, 
         SEXP_Xi, SEXP_Xi_gam, SEXP_Xi_sig, SEXP_Yi, SEXP_Yilag, SEXP_GAM, SEXP_SIG, 
         SEXP_countYs, SEXP_Offset, SEXP_etaitmp;
    ...
    for(i = 0; i < NumSubj; i++) {
        ...
        SEXP_Offset    = VECTOR_ELT(SEXP_subject, 6); 
        ...
        double dli1_dtheta[p_ALL]; 
        double li1;

        for(l = 0; l < p_ALL; l++) dli1_dtheta[l] = 0;
        ...
        for(e = 0; e < p_ALL; e++) ddtheta_loglikei[i*p_ALL + e] = dli1_dtheta[e];
